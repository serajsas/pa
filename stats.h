...
class stats {
 ...
 public:
  ...
  void get_Sum_Helper(PNG &im,int width,int height);
  double compute_var(char channel,pair<int,int> ul, int w, int h);
  long handle_get_sum(vector< vector< long >> &sumVect,pair<int,int> ul,int w,int h);
};
